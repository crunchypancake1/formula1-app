...
# uint8 ersDeployMode, 4×float (ersHarvested*, ersHarvestLimitPerLap, ersDeployed),
# uint8 networkPaused
...
def unpack_car_status(packet_header: PacketHeader, data: bytes) -> CarStatusPacket:
    """Unpack Car Status packet (Packet ID: 7). MAX_CARS cars of status data."""
    car_data_bytes = data[:(_CAR_STATUS_FORMAT_SIZE * MAX_CARS)]
    ...
    for f in struct.iter_unpack(_CAR_STATUS_FORMAT, car_data_bytes):
        ...
        # 22=ersHarvestedMGUH, 23=ersHarvestLimitPerLap, 24=ersDeployedThisLap,
        # 25=networkPaused
