...
class TestMotionParser:
    ...
    def test_max_cars_parsed(self):
        ...
        assert len(result.car_motion_data) == MAX_CARS
    ...
    def test_g_force_fields_quantised(self):
        # int16(1234) / 1000.0 == 1.234 -- assert the quantisation divide,
        # not just arbitrary floats, so an index-shift or missing-divisor
        # bug would fail this test.
        packet = _build_full_packet(g_lat=1234, g_long=-800, g_vert=9810)
        header = unpack_packet_header(packet[:PACKET_HEADER_FORMAT_SIZE])
        body = packet[PACKET_HEADER_FORMAT_SIZE:]
        result = unpack_motion(header, body)
        car = result.car_motion_data[0]
        assert car.g_force_lateral == pytest.approx(1.234)
